...
function rho(x,y,R,a)
    ...
    if a<1.1 #alpha=2 Hurst param TODO: check if this is correct, literature says a<=1.5, we used a<=1.1
        beta=0
        c_2=a/2
        c_0=1-a/2
    else
...
function FBM_simu_fast_double_param(;param_a::Parameter,param_b::Parameter, grid::Grid,num_sim::Int)::Tuple{Vector{Vector{Float64}},Vector{Vector{Float64}}}
    gridsize=grid.gridsize
    β_max=max(param_a.β,param_b.β)
    c_sqrt_a=sqrt(param_a.c)
    c_sqrt_b=sqrt(param_b.c)
    H_a=param_a.β/2 #Hurst Params
    H_b=param_b.β/2
    if β_max<1.1 #TODO: check if this is correct, literature says a<=1.5, we used a<=1.1
        R=1
    else
        R=2 #expanded gridsize, region of interest is [0,1]×[0.1]
    end
    ...
    for i in 1:n 
        for j in 1:m
            Rows_a[j,i]=rho([tx[i],ty[j]],[tx[1],ty[1]],R,2*H_a)[1]
            Rows_b[j,i]=rho([tx[i],ty[j]],[tx[1],ty[1]],R,2*H_b)[1]
        end
    end
    BlkCirc_row_a=[Rows_a  Rows_a[:,end-1:-1:2] ;  Rows_a[end-1:-1:2,:]  Rows_a[end-1:-1:2,end-1:-1:2 ]  ]
    BlkCirc_row_b=[Rows_b  Rows_b[:,end-1:-1:2] ;  Rows_b[end-1:-1:2,:]  Rows_b[end-1:-1:2,end-1:-1:2 ]  ]
    #calculate eigenvalues via fft
    eig_vals_a=real(fft2(BlkCirc_row_a)/(4*(m-1)*(n-1)))
    eig_vals_b=real(fft2(BlkCirc_row_b)/(4*(m-1)*(n-1)))
    #optional:
    #set small values to zero:
    #eps = 10^-8
    #eig_vals=eig_vals.*(abs.(eig_vals) .> eps)
    #eig_vals=eig_vals.*(eig_vals .>= 0)
    eig_vals_a=sqrt.(eig_vals_a)
    eig_vals_b=sqrt.(eig_vals_b)

    res_a=[Vector{Float64}(undef,gridsize*gridsize) for i in 1:num_sim]
    res_b=[Vector{Float64}(undef,gridsize*gridsize) for i in 1:num_sim]
    #res2=[Vector{Float64}(undef,gridsize*gridsize) for i in 1:num_sim]
    #one can get two times as many simulations for free by using the imaginary and real part 
    #of the complex gaussian, but they are dependend
    ...
    for trial in 1:(num_sim÷2)
        ...
        Z= randn(2*(m-1),2*(n-1)) + im* randn(2*(m-1),2*(n-1)) 
        #fft to calculate diag*Z
        F_a=fft2(eig_vals_a.*Z)
        F_b=fft2(eig_vals_b.*Z)
        #extract subblock with desired cov variance
        F_a=F_a[1:gridsize,1:gridsize]
        F_b=F_b[1:gridsize,1:gridsize]
        (out_a,c_0_a,c_2_a)=rho([0,0],[0,0],R,2*H_a)
        (out_b,c_0_b,c_2_b)=rho([0,0],[0,0],R,2*H_b)
        #optional two (dependend) real fields
        field1_a=real(F_a)
        field2_a=imag(F_a)
        field1_b=real(F_b)
        field2_b=imag(F_b)
        #set field zero at origin
        field1_a=field1_a.-field1_a[1,1]
        field2_a=field2_a.-field2_a[1,1]
        field1_b=field1_b.-field1_b[1,1]
        field2_b=field2_b.-field2_b[1,1]
        #TODO
        #make correction for embedding with a term c_2*r^2
        X_grid_comp = [i for i in tx[1:gridsize], j in tx[1:gridsize]].-tx[1]
        Y_grid_comp = [j for i in tx[1:gridsize], j in tx[1:gridsize]].-tx[1]
